    ...
    const loadData = async () => {
      try {
        ...
        const [quarterlyRes, monthlyRes] = await Promise.all([
          axios.get(`http://localhost:8001/api/reports/quarterly${query}`),
          axios.get(`http://localhost:8001/api/reports/monthly-trends${query}`)
        ])

        quarterlyData.value = quarterlyRes.data
