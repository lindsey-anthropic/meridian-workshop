...
export default {
  ...
  setup() {
    ...
    const calculateSummaryStats = () => {
      let total = 0
      for (const month of monthlyData.value) {
        total += month.revenue
      }
    ...
    const loadData = async () => {
      try {
        ...
        quarterlyData.value = await api.getReportsQuarterly(filters)
        monthlyData.value = await api.getReportsMonthlyTrends(filters)
    ...
    const formatMonth = (monthStr) => {
      // Guard against malformed dates rather than assuming YYYY-MM
    ...
    onBeforeUnmount(() => {
      if (stopFilterWatch) stopFilterWatch()
    })
