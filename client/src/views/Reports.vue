  ...
  <div class="reports">
    ...
    <div v-else>
      ...
      <div class="card">
        ...
        <div v-if="quarterlyData.length === 0" class="empty-state">{{ t('common.noData') }}</div>
      ...
      <div class="card">
        ...
        <div v-if="monthlyData.length === 0" class="empty-state">{{ t('common.noData') }}</div>
              ...
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                ...
                <td>
                  <span v-if="index > 0" :class="changeClass(month.revenue, monthlyData[index - 1].revenue)">
                    {{ formatChange(month.revenue, monthlyData[index - 1].revenue) }}
                  </span>
                  <span v-else>-</span>
                </td>
                <td>
...
export default {
  ...
  setup() {
    ...
    const totalRevenue = computed(() =>
      monthlyData.value.reduce((sum, m) => sum + safeNumber(m.revenue), 0)
    )
    ...
    const totalOrders = computed(() =>
      monthlyData.value.reduce((sum, m) => sum + safeNumber(m.order_count), 0)
    )

    const bestQuarter = computed(() => {
      if (quarterlyData.value.length === 0) return ''
      return quarterlyData.value.reduce((best, q) =>
        safeNumber(q.total_revenue) > safeNumber(best.total_revenue) ? q : best
      ).quarter
    })

    const maxMonthlyRevenue = computed(() =>
      monthlyData.value.reduce((max, m) => Math.max(max, safeNumber(m.revenue)), 0)
    )

    const barHeight = (revenue) => {
      const max = maxMonthlyRevenue.value
      if (max === 0) return 0
      return (safeNumber(revenue) / max) * 200
    }
    ...
    const getFulfillmentClass = (rate) => {
      ...
      if (r >= 90) return 'badge success'
      if (r >= 75) return 'badge warning'
      return 'badge danger'
    }
    ...
    const formatChange = (current, previous) => {
      ...
      return sign + formatCurrency(Math.abs(diff), currentCurrency.value)
    }
    ...
    const growthRate = (current, previous) => {
      ...
      const rate = ((safeNumber(current) - prev) / prev) * 100
      const sign = rate > 0 ? '+' : ''
