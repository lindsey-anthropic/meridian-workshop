...
export const api = {
  ...
  async getRestocking(budget = null, filters = {}) {
    const params = new URLSearchParams()
  ...
  async getReports(filters = {}) {
    ...
    const [quarterlyRes, monthlyRes] = await Promise.all([
      axios.get(`${API_BASE_URL}/reports/quarterly?${params.toString()}`),
      axios.get(`${API_BASE_URL}/reports/monthly-trends?${params.toString()}`)
    ])
    return { quarterly: quarterlyRes.data, monthly: monthlyRes.data }
  }
}
